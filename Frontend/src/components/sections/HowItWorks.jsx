...
import { PIPELINE_STEPS } from './pipeline/pipelineSteps';
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding: '30px 24px', display: 'flex', flexDirection: 'column', gap: '16px'
                }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: 'rgba(245, 158, 11, 0.12)', border: '1px solid rgba(245, 158, 11, 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon size={22} color="#f59e0b" />
                  </div>
                  <span style={{ fontSize: '1.4rem', fontWeight: 800, color: 'rgba(255, 255, 255, 0.15)', fontFamily: 'monospace' }}>{s.num}</span>
                </div>
                <h3 style={{ fontSize: '1.15rem', fontWeight: 700, color: '#ffffff' }}>{s.title}</h3>
                <p style={{ fontSize: '0.85rem', color: '#94a3b8', lineHeight: 1.6, margin: 0 }}>{s.description}</p>
              </div>
            );
          })}
        </div>
